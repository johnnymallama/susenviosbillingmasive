...
public interface NumeracionNcNdRepository extends CrudRepository<NumeracionNcNd, String> {
	...
	public Integer findProximoConsecutivo(@Param("pi_Tipo_Documento") String tipoDocumento);

	@Query(nativeQuery = true, value = "SELECT a.* FROM FE_NUMERACION_NC_ND a WHERE a.prefijo = SUBSTRING(?1,0,LEN(a.prefijo)+1) ")
	public NumeracionNcNd findNumeracionNcNdNumberDocument(String numberDocument);
